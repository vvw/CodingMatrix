...
def lin_comb_mat_vec_mult(M, v):
    ...
    '''
    assert(M.D[1] == v.D)
    mat_cols = mat2coldict(M)
    return sum(v[c] * mat_cols[c] for c in v.D)
...
def lin_comb_vec_mat_mult(v, M):
    ...
    '''
    assert(v.D == M.D[0])
    mat_rows = mat2rowdict(M)
    return sum(v[r] * mat_rows[r] for r in v.D)
...
def dot_product_mat_vec_mult(M, v):
    ...
    assert(M.D[1] == v.D)
    mat_rows = mat2rowdict(M)
    return Vec(M.D[0], {k:(mrow * v) for k, mrow in mat_rows.items()})
...
def dot_product_vec_mat_mult(v, M):
    ...
    assert(v.D == M.D[0])
    mat_cols = mat2coldict(M)
    return Vec(M.D[1], {k:(mcol * v) for k, mcol in mat_cols.items()})
...
solving_systems_m = Mat(({0, 1}, {0, 1}), {(0,0):-1/5.0, (0,1):4/5.0, (1,0):2/5.0, (1,1):-3/5.0})
